...
Есть веб-демо — та же система с прогрессом агентов в браузере: [Веб-демо](#веб-демо).
...
├── web/
│   └── static/                # Страница веб-демо (HTML, CSS, JS)
├── serve.py                   # Веб-демо: HTTP-сервер поверх Pipeline
├── api/
│   └── index.py               # Та же демка как функция Vercel (пошаговый режим)
├── examples/
...
├── Dockerfile
├── docker-compose.yml
├── vercel.json                # Конфиг деплоя: статика + функция
├── requirements.txt           # Зависимости рантайма для Vercel
└── .env.example
```
...
Мини-демо для показа работы системы: вводишь специальность — видишь, как агенты отрабатывают один за другим, с временем и токенами по каждому, а затем готовый отчёт по вкладкам.
...
| Блок                    | Что показывает                                                                                   |
|-------------------------|--------------------------------------------------------------------------------------------------|
| **Запустить анализ**    | Ввод роли, опционально уровень и цель. Общая роль («программист») уточняется через `role_clarifier` — варианты появляются прямо под полем |
| **Pipeline**            | Пять агентов живьём: ожидает → в работе → готово, время и токены по каждому агенту                |
| **Отчёт**               | Навыки, зарплаты, план на 90 дней, портфолио, разбор оценки критика, исходные `report.md`/`report.json` с кнопками скачивания |
| **Готовые отчёты**      | `examples/TC01–TC03` — открываются мгновенно, без обращений к LLM и без ключей                     |
| **Статистика запусков** | Агрегаты из `stats.json`: среднее время, токены, success rate, среднее по агентам, последние запуски |

Отчёты живых запусков складываются в `results/web/<дата-время>-<роль>/`, история пишется в тот же `stats.json`, что и у CLI.
...
| `--port`       | `8000`         | Порт                                  |
| `--output`     | `results/web`  | Куда складывать отчёты запусков       |
| `--stats`      | `stats.json`   | Путь к файлу статистики               |
...
```bash
docker compose up career-analyser-web
```

Демо поднимется на `http://localhost:8000`.

### Как это устроено

`serve.py` — тонкая обёртка над тем же `Pipeline`, без новых зависимостей (только стандартная библиотека). Запуск идёт в отдельном потоке, страница опрашивает `/api/run/<id>` и отрисовывает события. Прогресс берётся из необязательного колбэка `Pipeline(on_event=...)` — CLI работает как раньше, колбэк ему просто не передаётся.

| Эндпоинт                | Метод | Назначение                                        |
|-------------------------|-------|---------------------------------------------------|
| `/api/config`           | GET   | Готов ли LLM, список агентов, список примеров      |
| `/api/clarify`          | POST  | Уточнение неоднозначной роли                       |
| `/api/run`              | POST  | Старт pipeline, возвращает `run_id`                |
| `/api/run/<id>?since=N` | GET   | Новые события запуска, по готовности — весь отчёт   |
| `/api/examples/<id>`    | GET   | Готовый отчёт из `examples/`                       |
| `/api/stats`            | GET   | Агрегаты и последние запуски из `stats.json`       |
| `/api/agent`            | POST  | Один агент на присланном контексте (пошаговый режим) |
| `/api/finish`           | POST  | Сборка `report.md` из готового контекста             |

Без ключей в `.env` живой запуск отключается, но готовые отчёты и статистика остаются — демо можно показывать в офлайне.

### Деплой на Vercel

В проекте уже лежит всё нужное: `vercel.json`, `requirements.txt`, `.vercelignore` и функция `api/index.py`.
...
Или через сайт: **Add New → Project → Import Git Repository**, настройки менять не нужно — их берёт `vercel.json`.

Переменные окружения (**Project → Settings → Environment Variables**):

| Переменная          | Обязательная | Зачем                                                              |
|---------------------|--------------|--------------------------------------------------------------------|
| `GROQ_KEYS`         | да\*         | Ключи Groq через запятую                                           |
| `GROQ_MODELS`       | да\*         | Модели от лучшей к худшей                                          |
| `OPENROUTER_KEYS`   | да\*         | Ключи OpenRouter через запятую                                     |
| `OPENROUTER_MODELS` | да\*         | Модели от лучшей к худшей                                          |
| `DEMO_TOKEN`        | нет          | Живой запуск только по ссылке `?token=...`, примеры открыты всем    |
| `DEMO_ALLOW_RUN`    | нет          | `0` — выключить живые запуски совсем, оставить готовые отчёты       |
| `DEMO_MAX_RUNS`     | нет          | Запусков с одного IP за окно, по умолчанию `8`                      |
| `DEMO_WINDOW_SEC`   | нет          | Длина окна в секундах, по умолчанию `600`                           |
| `TEMPERATURE`, `MAX_TOKENS`, `MAX_RETRIES` | нет | Как в `.env`. Пустое значение считается незаданным — берутся `0.3`, `4096`, `3` |

\* хотя бы один провайдер; со вторым работает fallback. Без ключей демо поднимется в режиме «только готовые отчёты».

**Демо работает на ваших ключах и тратит ваши токены.** Для публичной ссылки поставьте `DEMO_TOKEN` и давайте ссылку вида `https://<проект>.vercel.app/?token=...` — без токена страница показывает отчёты из `examples/` и статистику, а кнопка запуска выключена.

### Чем деплой отличается от локального сервера

Лимит функции на Vercel — 60 секунд, а полный прогон занимает 58 секунд в медиане и до 87 в худшем случае (по `stats.json`). Поэтому на Vercel агенты вызываются **по одному запросу на агента**: контекст между шагами хранит браузер и присылает обратно. Живой прогресс по агентам при этом сохраняется — он и есть смысл демо.

| | Локально (`serve.py`) | Vercel (`api/index.py`) |
|---|---|---|
| Прогон       | один поток на сервере, страница опрашивает `/api/run/<id>` | по одному запросу на агента, контекст носит браузер |
| Отчёты       | пишутся в `results/web/<дата-время>-<роль>/`            | не пишутся — файловая система read-only, только кнопки «скачать» |
| `stats.json` | обновляется после каждого запуска                        | только читается из репозитория, история не растёт |
| Логика шага  | `serve.run_single_agent`                                  | тот же `serve.run_single_agent`                     |
| Статика      | `web/static` раздаёт сервер                               | её раздаёт сама функция — Vercel ведёт в неё все пути |
...
Маршрутизация: Vercel отдаёт функции все пути, включая `/`, поэтому страницу из `web/static` раздаёт сама функция. Сам файл `api/index.py` при этом доступен как `/api`, а не как `/api/index` — на это и ведут `rewrites` в `vercel.json`. Роут функция понимает и из `?route=`, и из чистого пути (`/api/config`), так что работает в обоих режимах маршрутизации.
